...
if __name__ == "__main__":
    moo = AESModeOfOperation()
    cleartext = open("2015_10_01_18_57_40_gonzalo.diez.enc",'r').read()

    key = open("2015_10_01_18_57_40_gonzalo.diez.key",'r').read()
    cypherkey = [ ord(c) for c in key]
    text = [ ord(c) for c in cleartext]
    # iv = [103,35,148,239,76,213,47,118,255,222,123,176,106,134,98,92]
    iv = [0 for a in range(0,16)]
    # mode, orig_len, ciph = moo.encrypt(cleartext, moo.modeOfOperation["CBC"],
    #         cypherkey, moo.aes.keySize["SIZE_256"], iv)

    decr = moo.decrypt(text, len(cleartext), moo.modeOfOperation["CBC"], cypherkey,
            moo.aes.keySize["SIZE_256"], iv)

    last = len(decr)

    for x in decr[last-1]:
        print x
